...
- [2.0 Teardown](#20-teardown)
- [3.0 Troubleshooting](#30-troubleshooting)
<!-- tocstop -->
...
Grafana opens straight on **"Isotope — stateless reports (Micrometer → Prometheus)"** (anonymous Admin, no login). Confirm scraping is live at Prometheus → [Status → Targets](http://localhost:9090/targets): each `isotope-stages` target should be **UP**. A target is `DOWN` until you start the stage on its port — that is expected.
