...
The standalone SQL walkthrough — a `CREATE MODEL` + `ML_PREDICT` PoC with provider notes and alternative options — is in [sql/ccaf-ai/trace_rca.fql](sql/ccaf-ai/trace_rca.fql). See also [root README §4.5](../../README.md#45-flink-sql-reports-on-confluent-cloud-for-apache-flink-ccaf).
