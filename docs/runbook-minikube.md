...
- **Pods stuck in `ErrImagePull` / `ImagePullBackOff` with `TLS handshake timeout`.** The minikube VM can't reach Docker Hub directly, usually because of a network filter or VPN on the host. Work around it with `MINIKUBE_HTTP_PROXY` — cause and workaround in [KNOWN_ISSUES.md](../KNOWN_ISSUES.md#20-every-image-pull-fails-with-tls-handshake-timeout-errimagepull--imagepullbackoff).
