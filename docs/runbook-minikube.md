...
- **`make minikube-start` says the existing cluster uses `driver=docker`.** The cluster predates the switch to VM drivers + containerd, and minikube can't convert it in place — `make minikube-delete && make cp-up`.
- **`make minikube-driver-check` fails.** It names the missing piece: `vfkit` (macOS), or on Linux `/dev/kvm` access, libvirt (`kvm2`), or `qemu-system-*` / `qemu-img` (`qemu`). Fix it with `make install-prereqs` (`sudo` on Linux) and log out and back in after any group change.
- **Pods stuck in `ErrImagePull` / `ImagePullBackOff` with `TLS handshake timeout`.** minikube can't reach Docker Hub directly (on the legacy Docker driver, usually a host VPN with Docker Desktop's proxy in play; on the VM driver, usually a host network filter, which you can work around with `MINIKUBE_HTTP_PROXY`) — cause and workaround in [KNOWN_ISSUES.md](../KNOWN_ISSUES.md#20-every-image-pull-fails-with-tls-handshake-timeout-errimagepull--imagepullbackoff).
