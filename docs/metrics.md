...
Emission lives in one place — [PrometheusIsotopeMetrics.java](https://github.com/j3-signalroom/kafka-isotope/blob/main/kafka-isotope-metrics/src/main/java/ai/signalroom/kafka/isotope/metrics/PrometheusIsotopeMetrics.java) — mirroring how `TDigests` centralizes the sketch contract.
...
Point a Prometheus scrape job at each stage's `/metrics` and the three reports become PromQL queries (no report topics, no Control Center — Grafana instead). Unlike the Flink jobs, there's **no watermark wait**: cumulative counters update on every send and Prometheus windows them at read time.

> **Running the Grafana showcase? Add `-Dmetrics.prometheus.port`.** The command above binds the default `9404`, which is right for a single stage you scrape by hand — but stages share one host, so they cannot all bind it, and [§8.0](#80-one-command-showcase-prometheus--grafana-on-minikube)'s Prometheus config scrapes a fixed port per stage: `enrich`→`9410`, `fulfill`→`9411`, `ship`→`9412`. Omit the flag and the stage still exports happily on `9404` while every Prometheus target reads `connection refused` and Grafana shows no data. Check with `kubectl port-forward -n monitoring svc/prometheus 9090:9090` → Status → Targets.
...
> The same **backlog caveat** from [Enabling the exporter](#20-enabling-the-exporter) applies: `hop` / `consume` / `ship` use a random consumer group with `auto.offset.reset=earliest`, so a fresh stage replays from offset 0 and both `isotope_consume_latency_seconds_sum` and `isotope_consume_age_seconds_sum` then reflect days-old origins, not steady-state. Read the windowed `rate(...sum[1m]) / rate(...count[1m])`, or start with `-Disotope.consume.from=latest`.
...
| Which input records produced this merged output? | 🔴 | **Opt-in** fan-in provenance ([flink-collector.md §2.4](./flink-collector.md#24-optional-fan-in-provenance)): join `orders.flink_batched` to `isotope_merge_edge_markers` on `merge_trace_id` for the whole parent set — `contributing_trace_id` / `_service` / `_topic` / `_records`, one row per contributing trace per window (the row count equals the merged record's `distinct_traces`; `SUM(contributing_records)` equals its `event_count`). No aggregate ever forwards a parent's trace, so this is the *only* way to recover the derivation. |
...
| Which versions produced the current value of this row? | 🔴 | **Opt-in**, CP-only state-level provenance (root README §3.5.3): `isotope_state_provenance` carries one record per emitted state, content-addressed by `StateVersion`, with the versions it derived from inline in `parents`. This is the one question the windowed merge collector cannot reach at all — an unbounded `GROUP BY`, a regular join, and a dedup have no window bounds to key an identity on. |
